...
func TestDialerGroup_Select_Fallback(t *testing.T) {
	...
	// Recovery is asserted at the set level in
	// TestAliveDialerSet_GetFirstAliveFollowsGroupOrder, which can drive a real
	// revival: marking a collection available is not reachable from here.
}
...
func TestDialerGroup_Select_FallbackExcludesFailedDialer(t *testing.T) {
	...
	d, _, _, err := g.SelectWithExclusionResult(TestNetworkType, false, dialers[0])
	if err != nil {
		t.Fatal(err)
	}
	if d != dialers[1] {
...
func newTestGroupWithAllDialersDead(t *testing.T, policy consts.DialerSelectionPolicy, count int) (*DialerGroup, []*dialer.Dialer) {
	...
	for range count {
		dialers = append(dialers, newDirectDialer(option))
	}
	g := NewDialerGroup(option, "test-group", dialers, newEmptyAnnotations(len(dialers)),
...
func TestDialerGroup_Select_FallbackLastResortSkipsExcluded(t *testing.T) {
	...
	d, _, _, err = single.SelectWithExclusionResult(TestNetworkType, false, only)
	if err != nil || d != only {
