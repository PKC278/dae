...
func (m *RoutingMatcher) MatchWithDrop(
	...
	for i, match := range matches {
		...
		if match.matchType == consts.MatchType_ProcessName && facts.pname[0] == 0 {
			// LAN traffic carries no process metadata, so a pname
			// rule, inverted or not, must not decide it. Skip the
			// whole rule and let later rules match.
			badRule = true
		}
