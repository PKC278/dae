...
func TestDomainRoutingTrackerMergesSharedIPAcrossOwners(t *testing.T) {
	...
	ip16 := ip.As16()
	ipKey := bpfRoutingEpochIp{Addr: common.Ipv6ByteSliceToUint32Array(ip16[:])}
...
func TestDomainRoutingTrackerReplacesOwnerSnapshotWithoutLeakingRefs(t *testing.T) {
	...
	ip20 := bpfRoutingEpochIp{Addr: common.Ipv6ByteSliceToUint32Array(ip20Bytes[:])}
	ip21Addr := netip.MustParseAddr("203.0.113.21")
	ip21Bytes := ip21Addr.As16()
	ip21 := bpfRoutingEpochIp{Addr: common.Ipv6ByteSliceToUint32Array(ip21Bytes[:])}
