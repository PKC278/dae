...
var SectionSummaryDesc = Desc{
	...
	"rule_provider": "Remote mihomo-compatible rule sets referenced by routing rules as rule-set:name.\nExample: cn:\"https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/meta/geo/geosite/cn.list\"",
...
Built-in outbound: direct, must_direct, block. Use block(drop) to silently drop packets; plain block explicitly rejects the connection.
...
4. "domain++". Based on domain+ mode but force to re-route traffic using sniffed domain to partially recover domain based traffic split ability. It doesn't work for direct traffic and consumes more CPU resources.`,
	...
	"rule_provider_update_interval": "Automatic update interval for remote rule_provider rule sets. Default is 0, which disables automatic updates. Supported units are s for seconds and d for days.",
